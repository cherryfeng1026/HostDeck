...
func TestServerRoutes_CRUD(t *testing.T) {
	...
	createReq := httptest.NewRequest(
		http.MethodPost,
		"/api/servers",
		bytes.NewBufferString(`{"name":"prod-web-01","hostname":"prod-web-01","ip":"10.0.0.21","sshPort":22,"username":"root","authType":"password","password":"super-secret","trustedHostKeyFingerprint":"SHA256:trusted-create","collectorMode":"ssh_only","tags":["prod","web"],"expiresAt":"2026-06-21T01:00:00Z","maintenanceStartAt":"2026-04-21T01:00:00Z","maintenanceEndAt":"2026-04-21T03:00:00Z"}`),
	)
	...
	if items[0].ExpiresAt != "2026-06-21T01:00:00Z" {
		t.Fatalf("unexpected expiresAt after create: %+v", items[0])
	}
	...
	updateReq := httptest.NewRequest(
		http.MethodPut,
		"/api/servers/1",
		bytes.NewBufferString(`{"name":"prod-web-01","hostname":"prod-web-01","ip":"10.0.0.22","sshPort":2222,"username":"admin","authType":"password","trustedHostKeyFingerprint":"SHA256:trusted-update","collectorMode":"prefer_agent","tags":["prod"],"expiresAt":"2026-07-22T01:00:00Z","maintenanceStartAt":"2026-04-22T01:00:00Z","maintenanceEndAt":"2026-04-22T04:00:00Z","enabled":false}`),
	)
	...
	if items[0].ExpiresAt != "2026-07-22T01:00:00Z" {
		t.Fatalf("unexpected expiresAt after update: %+v", items[0])
	}
	if items[0].Enabled {
...
func TestServerRoutes_UpdateMissingReturnsNotFound(t *testing.T) {
	db := openAPITestDB(t)
	repo := storage.NewServerRepository(db, "test-master-key")
	router := httpx.NewRouterWithHandlers(api.NewServerHandler(repo, nil), nil, nil, nil, nil, nil)
	...
	req := httptest.NewRequest(
		http.MethodPut,
		"/api/servers/999",
		bytes.NewBufferString(`{"name":"missing","hostname":"missing","ip":"10.0.0.99","sshPort":22,"username":"root","authType":"password","collectorMode":"ssh_only"}`),
	)
...
func TestServerRoutes_DeleteMissingReturnsNotFound(t *testing.T) {
	db := openAPITestDB(t)
	repo := storage.NewServerRepository(db, "test-master-key")
	router := httpx.NewRouterWithHandlers(api.NewServerHandler(repo, nil), nil, nil, nil, nil, nil)
...
func TestServerRoutes_CreateSupportsPrivateKeyAuth(t *testing.T) {
	db := openAPITestDB(t)
	repo := storage.NewServerRepository(db, "test-master-key")
	router := httpx.NewRouterWithHandlers(api.NewServerHandler(repo, nil), nil, nil, nil, nil, nil)
	...
	req := httptest.NewRequest(
		http.MethodPost,
		"/api/servers",
		bytes.NewBufferString(`{"name":"prod-web-01","hostname":"prod-web-01","ip":"10.0.0.21","username":"root","authType":"private_key","privateKey":"-----BEGIN OPENSSH PRIVATE KEY-----\ntest\n-----END OPENSSH PRIVATE KEY-----","collectorMode":"ssh_only"}`),
	)
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)
	...
	if items[0].PasswordConfigured {
		t.Fatalf("expected passwordConfigured=false for key auth")
	}
	if !items[0].PrivateKeyConfigured {
		t.Fatalf("expected privateKeyConfigured=true for key auth")
	}
}

func TestServerRoutes_CreateRequiresPrivateKeyWhenUsingKeyAuth(t *testing.T) {
	db := openAPITestDB(t)
	repo := storage.NewServerRepository(db, "test-master-key")
	router := httpx.NewRouterWithHandlers(api.NewServerHandler(repo, nil), nil, nil, nil, nil, nil)
	...
	req := httptest.NewRequest(
		http.MethodPost,
		"/api/servers",
		bytes.NewBufferString(`{"name":"prod-web-01","hostname":"prod-web-01","ip":"10.0.0.21","username":"root","authType":"private_key","collectorMode":"ssh_only"}`),
	)
	req.Header.Set("Content-Type", "application/json")
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)
	...
	if payload["error"] != "SSH 私钥不能为空" {
		t.Fatalf("unexpected error message: %q", payload["error"])
	}
}

func TestServerRoutes_UpdateRejectsAuthTypeSwitchWithoutCredential(t *testing.T) {
	db := openAPITestDB(t)
	repo := storage.NewServerRepository(db, "test-master-key")
	router := httpx.NewRouterWithHandlers(api.NewServerHandler(repo, nil), nil, nil, nil, nil, nil)
	...
	updateReq := httptest.NewRequest(
		http.MethodPut,
		"/api/servers/1",
		bytes.NewBufferString(`{"name":"prod-web-01","hostname":"prod-web-01","ip":"10.0.0.21","sshPort":22,"username":"root","authType":"private_key","collectorMode":"ssh_only"}`),
	)
	updateReq.Header.Set("Content-Type", "application/json")
	updateRec := httptest.NewRecorder()
	router.ServeHTTP(updateRec, updateReq)
	...
	if updateRec.Code != http.StatusBadRequest {
		t.Fatalf("expected update status %d, got %d body=%s", http.StatusBadRequest, updateRec.Code, updateRec.Body.String())
	}
	items := requestServerList(t, router, "/api/servers")
	if len(items) != 1 {
		t.Fatalf("expected 1 server, got %d", len(items))
